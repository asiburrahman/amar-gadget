...
export const ElectroHeader: React.FC = () => {
  ...
  return (
    <header className="w-full bg-white text-[#333e48] border-b border-gray-200 z-50 sticky top-0 shadow-xs">
      {/* 1. TOPBAR */}
      <div className="hidden lg:block bg-white border-b border-gray-100 text-xs text-gray-500 py-2.5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-gray-500 font-medium">Welcome to Amar Gadget Electronics Store</span>
          </div>

          <div className="flex items-center space-x-6 font-medium">
            <Link href="/contact" className="hover:text-black flex items-center gap-1.5 transition-colors">
            ...
            <Link href="/track-order" className="hover:text-black flex items-center gap-1.5 transition-colors">
              <svg className="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center space-x-1 hover:text-black focus:outline-hidden cursor-pointer"
              >
            ...
            <Link href="/login" className="hover:text-black flex items-center gap-1.5 transition-colors">
              <User className="w-3.5 h-3.5 text-gray-500" />
              <span>Register <span className="text-gray-400 font-normal">or</span> Sign in</span>
            </Link>
          </div>
        </div>
      </div>

      {/* 2. MAIN HEADER (Logo, Hamburger, Search Bar, Compare/Wishlist/Cart) */}
      <div className="py-2.5 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4 lg:gap-8">
          ...
          <div className="flex items-center space-x-4">
            <Link href="/" className="flex items-center group">
              <div className="relative flex items-center">
                <span className="text-3xl font-black tracking-tight text-[#333e48]">
                  amar gadget
                </span>
                <span className="w-2.5 h-2.5 bg-[#fed700] rounded-full inline-block ml-0.5 mt-3 group-hover:scale-125 transition-transform"></span>
              </div>
            </Link>
          ...
          <div className="hidden md:flex flex-1 max-w-2xl mx-2">
            <form onSubmit={(e) => e.preventDefault()} className="flex items-center w-full h-11 rounded-full border-2 border-[#fed700] bg-white p-1 shadow-xs focus-within:ring-2 focus-within:ring-[#fed700]/50 transition-all">
              ...
              <input
                ...
                className="flex-1 pl-5 pr-3 py-1.5 text-sm text-[#333e48] placeholder-gray-400 focus:outline-hidden bg-transparent"
              />

              {/* Category Select Dropdown on Right */}
              <div className="relative border-l border-gray-200 pl-3 pr-2 flex items-center shrink-0">
                <select
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value)}
                  className="pl-1 pr-6 py-1.5 text-xs font-semibold text-gray-600 bg-transparent appearance-none cursor-pointer focus:outline-hidden"
                >
              ...
              <button
                type="submit"
                className="w-9 h-9 rounded-full bg-[#fed700] hover:bg-[#eec800] text-[#333e48] flex items-center justify-center shrink-0 transition-all cursor-pointer shadow-xs ml-1"
                aria-label="Search"
              >
                <Search className="w-4.5 h-4.5 text-[#333e48]" />
              </button>
            </form>
          </div>
          ...
          <div className="flex items-center space-x-6 lg:space-x-7">
            ...
            <Link href="/cart" className="flex items-center space-x-2 text-[#333e48] hover:opacity-90 transition-opacity">
              ...
              <span className="hidden sm:inline-block text-sm font-black text-[#333e48] ml-1">
        ...
        <div className="mt-3 px-4 md:hidden">
          <form onSubmit={(e) => e.preventDefault()} className="flex items-center w-full h-10 rounded-full border-2 border-[#fed700] bg-white p-1">
            <input
              type="text"
              placeholder="Search for Products"
              value={searchQuery}
            ...
            <button
              type="submit"
              className="w-8 h-8 rounded-full bg-[#fed700] text-[#333e48] flex items-center justify-center cursor-pointer shrink-0"
            >
      ...
      {/* 3. NAVIGATION BAR & DEPARTMENTS MENU */}
      <div className="bg-white border-t border-gray-200 h-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-full">
          ...
          <div className="flex items-center space-x-8 h-full">
            ...
            <div className="relative w-64 sm:w-72 shrink-0 h-full flex items-end">
              <button
                onClick={() => setIsDepartmentOpen(!isDepartmentOpen)}
                className="w-full h-full bg-[#fed700] hover:bg-[#eec800] text-[#333e48] font-bold text-xs sm:text-sm px-4 rounded-t-lg flex items-center space-x-2.5 transition-colors cursor-pointer"
              >
                ...
                >
                  {categories.map((cat, idx) => (
                    <Link
                      key={idx}
                      href={cat.href}
                      className={`flex items-center justify-between px-4 py-1.5 text-xs text-gray-700 hover:bg-gray-100 hover:text-black transition-colors ${
            ...
            <nav className="hidden lg:flex items-center space-x-6 text-xs sm:text-sm font-extrabold text-[#333e48] h-full">
              <Link href="/deals" className="text-[#DE4437] hover:text-red-700 flex items-center gap-1 transition-colors">
                <span>Super Deals</span>
                <ChevronDown className="w-3 h-3 text-[#DE4437]" />
              </Link>
              ...
              {categories.map((cat, idx) => (
                <Link
                  ...
                  className={`block px-3 py-2 text-sm rounded-lg hover:bg-gray-100 transition-colors ${
                    cat.isBold ? "font-bold text-[#333e48]" : "font-medium text-gray-700"
                  }`}
                >
                  {cat.name}
                </Link>
